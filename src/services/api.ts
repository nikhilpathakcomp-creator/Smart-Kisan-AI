...
export const api = {
  ...
  async getCropRecommendations(params: any) {
    const res = await fetch('/api/agriculture/crop-recommendation', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  async getIrrigationSchedule(params: any) {
    const res = await fetch('/api/agriculture/smart-irrigation', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(params),
    });
  ...
  async getFertilizerGuide(params: any) {
    const res = await fetch('/api/agriculture/fertilizer-recommendation', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(params),
    });
  ...
  async getMandiPrices(query?: string): Promise<MandiPriceItem[]> {
    const url = query ? `/api/mandi-prices?query=${encodeURIComponent(query)}` : '/api/mandi-prices';
    const res = await fetch(url);
    return res.json();
  },
